...
def laad_data(csv, batch_size, shuffle = True, distored = True):
    ...
    return _generate_image_and_label_batch(
        image,label,filename,min_queue_examples, batch_size, shuffle=shuffle)

def _generate_image_and_label_batch(image, label, filename, min_quenue_example,
                                    batch_size, shuffle):
